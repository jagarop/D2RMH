...
void ProcessData::updateOffset() {
    ...
    if (mem && READN(baseAddr, mem, uint32_t(baseSize))) {
        const uint8_t search0[] = {0x48, 0x8D, 0x0D, 0, 0, 0, 0, 0x48, 0xC1, 0xE0, 0x0A, 0x48, 0x03, 0xC1, 0xC3, 0xCC};
        ...
        if (off != size_t(-1)) {
            int32_t rel;
            if (READ(baseAddr + off + 5, rel)) {
                uiBaseAddr = baseAddr + rel;
            }
        }

        const uint8_t search2[] =
            {0xC7, 0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x48, 0x85, 0xC0, 0x0F, 0x84, 0x00, 0x00, 0x00,
             0x00, 0x83, 0x78, 0x5C, 0x00, 0x0F, 0x84, 0x00, 0x00, 0x00, 0x00, 0x33, 0xD2, 0x41};
        const uint8_t mask2[] =
            {0xFF, 0xFF, 0, 0, 0, 0, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0, 0, 0, 0, 0xFF, 0xFF, 0xFF,
             0xFF, 0xFF, 0xFF, 0, 0, 0, 0, 0xFF, 0xFF, 0xFF};
        off = searchMem(mem, size_t(baseSize), search2, mask2, sizeof(search2));
        if (off != size_t(-1)) {
            int32_t rel;
            if (READ(baseAddr + off - 4, rel)) {
                isExpansionAddr = baseAddr + off + rel;
            }
        }
        ...
        const uint8_t search4[] =
            {0xE8, 0x00, 0x00, 0x00, 0x00, 0x48, 0x8D, 0x0D, 0x00, 0x00, 0x00, 0x00, 0x44, 0x88, 0x2D, 0x00, 0x00, 0x00,
             0x00};
        ...
        if (off != size_t(-1)) {
            int32_t rel;
            if (READ(baseAddr + off + 8, rel)) {
                gameInfoAddr = baseAddr + off - 244 + rel;
            }
        }
    }
    delete[] mem;
...
void ProcessData::readGameInfo() {
    ...
    if (READ(gameInfoAddr, gameInfo)) {
        ...
        gameIP.assign(gameInfo.gameIPBuffer, gameInfo.gameIPBuffer + gameInfo.gameIP.size);
    } else {
        gameName.clear();
        gamePass.clear();
        region.clear();
